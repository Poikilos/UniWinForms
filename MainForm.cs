...
namespace ExpertMultimedia {
	...
	public class MainForm : System.Windows.Forms.Form {
		int iPersistentTLHeight=4;
		public static string sMyName="UniWinForms";
		public static string sMyVersion="";//loaded from 1.Version.txt
		...
		private System.Windows.Forms.MenuItem miEditSplit0;
		...
		public MainForm() {
			...
			splashscreen.sStatus=sMsg;
			//
			//Do init here:
			//splashscreen.sStatus=sMsg; //DOES check if different, but ONLY displays if has been more than iMinSplashRefreshDelay ms
			/*
			int iFakeMessageSpleeper=6;
			int iFakeMessages=(2000-(Environment.TickCount-splashscreen.iStartedTick))/iFakeMessageSpleeper;
			for (int iX=0; iX<iFakeMessages; iX++) {
				splashscreen.sStatus="Starting...ShowDelay "+iX.ToString()+" of "+iFakeMessages.ToString();
			...
			*/
			...
			FixSizes();
			iPersistentTLHeight=this.ClientRectangle.Height/2;//iPersistentTL=this.ClientRectangle.Height-(this.toolbar.Height+this.tbStatus.Height+this.treeview1.Height);
		}
		public static MainForm mainformNow=null;
		public static bool bChanges=false;
		public static SplashScreen splashscreen=null;
		public static UniWinFormsSolution solutionNow=null;
		[STAThread]
		...
		private void InitializeComponent() {
			//if (Environment.OSVersion.ToString().IndexOf("Win")<0) this.ControlBox = false; //since using Form Closing CancelEventArgs doesn't work in mono 1.2.2 (tested 2007-09-01) (if not Windows then assume linux and hide control box in order to prevent exit without asking to save)
			...
			this.miEditSplit0 = new System.Windows.Forms.MenuItem();
			...
			// miEditSplit0
			// 
			this.miEditSplit0.Index = 0;
			this.miEditSplit0.Text = "-";
			// 
			// miExport
			// 
			...
			// panelTL
			// 
			this.panelTL.BorderStyle = System.Windows.Forms.BorderStyle.Fixed3D;
			this.panelTL.BackColor = System.Drawing.SystemColors.Control;
			...
			this.Name = "MainForm";
			if (File.Exists("1.Version.txt")) {
				StreamReader trNow=null;
				try {
					trNow=new StreamReader("1.Version.txt");
					sMyVersion=trNow.ReadToEnd();
				}
				catch {}
				try {if (trNow!=null) trNow.Close();} catch {}
			}
			this.Text = sMyName+" "+sMyVersion;
		...
		{
			FixSizes();
			solutionNow=new UniWinFormsSolution();
		}
		...
		void MiOpenClick(object sender, System.EventArgs e)
		{
			...
			ofiledlgNow.Filter="Solution (*.sln)|CSharp Project (*.csproj)|CSharp Files (*.cs)|*.cs|All Files (*.*)|*.*"; //TODO: Solution File (*.cmbx)|Solution File (*.prjx)
			ofiledlgNow.ShowDialog();
			if (ofiledlgNow.FileName!="") {
				FileInfo fiNow=new FileInfo(ofiledlgNow.FileName);
				if (ofiledlgNow.FileName.EndsWith(".cs")) {
					DirectoryInfo diNow = new DirectoryInfo(fiNow.Path);
					UniWinForms uwfsProject=new UniWinForms();
					//FileInfo[] fiarrFiles = di.GetFiles("*.aspx");
					//string[] sarrFiles=null;
					//if (sarrFiles!=null) {
					//	sarrFiles=new string[fiarrFiles.Length];
					//	int iNow=0;
					//	foreach(FileInfo fi in fiarrFiles) {
					//		sarrFiles[iNow]=fi.Name;
					//		iNow++;
					//	}
					//}
					//uwfsProject.Load(sarrFiles);
					//TODO: load in tabs instead of project
				}
				
				solutionNow.Load(sFileName);//TODO: warn if already loaded
				mainformNow.cbWindow.BeginUpdate();
				mainformNow.cbWindow.Items.Clear();
				foreach (string sNow in solutionNow.FormNameStringsToObjectArray()) {
					mainformNow.cbWindow.Items.Add(sNow);
				}
				//mainformNow.cbWindow.Items.AddRange(solutionNow.FormNameStringsToObjectArray());
				mainformNow.cbWindow.SelectedIndex=0;
				mainformNow.cbWindow.EndUpdate();
			}
		}
		
		void MiSaveClick(object sender, System.EventArgs e)
		{
			if (sFileName!="") solutionNow.Save(sFileName);//TODO: warn if overwrite
			else MiSaveAsClick(sender,e);
		}
		...
		void MiSaveAsClick(object sender, System.EventArgs e)
		{
			...
			if (sfiledlgNow.FileName!="") {
				sFileName=sfiledlgNow.FileName;
				solutionNow.Save(sFileName);
			}
		}
	...
	//TODO: uwfNow.Close();
		...
		void MiUndoClick(object sender, System.EventArgs e)
		{
			 
		}
		void MiRedoClick(object sender, System.EventArgs e)
		{
		...
		{
			string sHelpFile="help/index.html";
			try {System.Diagnostics.Process.Start(sHelpFile); }
			catch (Exception exn) {
				MessageBox.Show("Unable to launch help file \""+sHelpFile+"\"");
			}
		...
		{
			iPersistentTLHeight=this.ClientRectangle.Height-(this.treeview1.Height+this.toolbar.Height+this.tbStatus.Height);//-splitterLR.Top;
			FixSizes();
		...
		public void FixSizes() {
			this.panelRight.Top=this.toolbar.Height;
			this.panelRight.Left=splitterLR.Left+splitterLR.Width;
			...
			//if (this.splitterUDBL.Height>(this.ClientRectangle.Height-(splitterUDBL.MinExtra+this.toolbar.Height+this.tbStatus.Height))) this.splitterUDBL.Height=this.ClientRectangle.Height-(splitterUDBL.MinExtra+this.toolbar.Height+this.tbStatus.Height+32);
			//if (this.splitterUDBL.Top<splitterUDBL.MinExtra) this.splitterUDBL.Top=splitterUDBL.MinExtra;
			treeview1.Height=this.ClientRectangle.Height-(iPersistentTLHeight+this.toolbar.Height+this.tbStatus.Height);
		}
		//void MainFormActivated(object sender, System.EventArgs e) {
    	//	splashscreen.Close();
		//}
	...
	how to use (ToString() does NOT work!):
	string sNow=Environment.GetFolderPath(Environment.SpecialFolder.Personal);
	*/
}
